...
本页对应设置页的 “General” 分组，以及它所承载的应用级状态和通用处理开关。它负责通用处理开关、API 参数文件开关、过滤列表、全局蒙版参数、模型卸载、任务完成后动作和编辑器偏好；检测、OCR、翻译、修复、排版、超分和上色的专用参数分别见对应设置页。
...
### 任务完成后 {#shutdown-after-translation}

“设置 → General”中的“任务完成后”下拉框提供：不执行操作（默认）、关闭软件、睡眠、休眠、关机。批次至少成功保存一个新文件、没有失败文件且临时文件清理结束后，开始 60 秒倒计时。手动停止、失败或全部跳过的批次不会触发动作；无法提交清理任务时也不会触发。

倒计时由软件管理，修改选项、开始新批次或点击停止即可在系统命令发出前取消。关闭软件使用正常窗口关闭流程；Windows 睡眠/休眠通过 .NET 电源 API 执行，Linux 使用 systemctl。macOS 支持睡眠和关机，不支持显式休眠，会记录不可用警告。电源动作需要操作系统支持及权限，失败会记录警告，不会修改系统电源配置。此设置只影响桌面端翻译批次。

默认值：`none`（`app.after_translation_action`）。
