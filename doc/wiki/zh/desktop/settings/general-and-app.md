...
“翻译完成后自动关机”开关位于“设置 → General”，默认关闭。开启后，桌面端在一个批次成功完成且后台清理任务空闲后，使用系统关机命令安排电脑在 60 秒后关机；这段缓冲时间用于保存结果，并留出取消待执行关机的时间。手动停止、批次级错误或单个文件失败时不会触发。如果系统权限不足或关机命令不可用，应用会记录警告并保持打开。

Windows 使用 `shutdown /s /t 60`，Linux/macOS 使用系统支持的延时关机命令。此设置只影响桌面端翻译批次，不会改变 CLI 或远程服务的生命周期。
